...
render_forgot_password :: proc(w: io.Writer, data: ^ForgotPasswordData) {
	...
	io.write_string(w, ".auth-footer { text-align: center; padding: 1.5rem; color: #999; font-size: 0.8rem; } </style> </head> <body> <div class=\"auth-header\"> <h1><a href=\"/\">ShopOdin</a></h1> </div> <div class=\"auth-wrap\"> <div class=\"auth-box\">")
	if len(data.error) > 0 {
		io.write_string(w, "<div class=\"error-msg\">")
		io.write_string(w, html_escape(data.error))
		io.write_string(w, "</div>")
	}
	if len(data.success) > 0 {
		io.write_string(w, "<div class=\"success-msg\">")
		io.write_string(w, html_escape(data.success))
		io.write_string(w, "</div>")
	}
	// {template "content"}
	io.write_string(w, "<h2>Reset Password</h2> <p class=\"subtitle\">Enter your email and we'll send you a reset link.</p> <form method=\"post\" action=\"/forgot-password\"> <div class=\"field\"> <label for=\"email\">Email</label> <input type=\"email\" id=\"email\" name=\"email\" placeholder=\"you@example.com\" value=\"")
	io.write_string(w, data.email)
	io.write_string(w, "\" required> </div> <button type=\"submit\" class=\"btn\">Send Reset Link</button> </form> <div class=\"auth-links\"> Remember your password? <a href=\"/login\">Sign in</a> </div>")
	io.write_string(w, "</div> </div> <div class=\"auth-footer\"> <p>©")
	buf_0: [32]u8
	io.write_string(w, write_int(buf_0[:], i64(data.year)))
	io.write_string(w, "ShopOdin</p> </div> </body> </html>")
}
